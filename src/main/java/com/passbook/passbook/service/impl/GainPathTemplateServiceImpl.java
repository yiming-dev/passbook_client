...
@Slf4j
@Service
public class GainPathTemplateServiceImpl implements IGainPassTemplateService{
    private final HbaseTemplate hbaseTemplate;
    private final StringRedisTemplate redisTemplate;

    @Autowired
    public GainPathTemplateServiceImpl(HbaseTemplate hbaseTemplate, StringRedisTemplate redisTemplate) {
    ...
    private boolean addPassForUser(GainPassTemplateRequest request,Integer merchantsId,String passTemplateId) throws Exception{
        ...
        if(request.getPassTemplate().isHasToken()){
            String token = redisTemplate.opsForSet().pop(passTemplateId);
            if(null == token){
                log.error("Token Not Wxist{}",passTemplateId);
                return  false;
            }
